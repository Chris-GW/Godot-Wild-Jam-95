...
func _ready() -> void:
	if DialogueProgress.is_complete(start_from_title):
		CustomLogger.debug("%s dialogue has already played, freeing..." % start_from_title)
		queue_free()
	else:
		SignalHelper.once(screen_entered, on_screen_entered)

func on_screen_entered() -> void:
	DialogueProgress.complete(start_from_title)
